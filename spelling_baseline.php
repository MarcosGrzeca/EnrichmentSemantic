...
$tweets = query("SELECT * FROM tweets WHERE situacao IN ('N', 'M')");
...
$ind = 0;
 foreach (getRows($tweets) as $key => $value) {
	...
	try {
		...
		// debug(array("erroParseado" => $totalErros, "textParser" => $originalText));
		update("tweets", $value["id"], array("erroParseado" => $totalErros, "textParser" => $originalText));
	} catch (Exception $e) {
		debug($e->getMessage());
	}
	if ($ind > 10) {
