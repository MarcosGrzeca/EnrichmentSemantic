...
$tweets = query("SELECT * FROM tweets_amazon WHERE preProcessado = 0");
...
foreach (getRows($tweets) as $key => $value) {
    ...
    try {
        ...
        $totalPositivo = 0;
        $totalNegativo = 0;
        if (checkEmoji2($textOriginal)) {
            $emoticons = getEmoticons($textOriginal);
        ...
        $fieldsAtualizado = ["preProcessado" => 1, "textParser" => $texto, "hashtags" => implode(",", $hashTags), "emoticonPos" => $totalPositivo, "emoticonNeg" => $totalNegativo];

        // if ($value["situacao"] == 1) {
            $fieldsAtualizado["textOriginal"] = $textOriginal;
